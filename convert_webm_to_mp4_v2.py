...
def convert_webm_to_mp4(webm_path: Path) -> bool:
    ...
    try:
        ...
        # Print stderr if there's an error (it may contain the ffmpeg output)
        if result.returncode != 0:
            stderr = result.stderr.strip()
            if stderr:
                print(f"    ❌ Error (code {result.returncode}): {stderr[:250]}...")
            else:
                print(f"    ❌ Error (code {result.returncode}): {result.stdout[:250]}...")
            return False
        
        # Check if output file was created and has size >0
        if not mp4_path.exists():
            print(f"    ❌ Error: MP4 file was not created")
            return False
        
        if mp4_path.stat().st_size == 0:
            print(f"    ❌ Error: MP4 file is empty")
            mp4_path.unlink()
            return False
...
def main():
    ...
    for i, webm_path in enumerate(webm_files, 1):
        print(f"[{i}/{total}] {webm_path.name}")
